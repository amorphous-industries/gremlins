...
6. **Verify with git diff, not grep**: after making a batch of edits, verify
   correctness with `git diff` or `git diff --cached` (for staged changes).
   This confirms every change in one step and avoids redundant re-reading.
   Only grep if you need to find a specific pattern across the codebase.
