...
5. Write an **updated plan document** (the "rolling plan") to `{rolling-plan}`
...
6. If exit state is **`next-plan`**, write a **child plan** to `{child-plan}`
...
7. Write the **signal marker** to `{signal}`
   - Valid JSON, exactly this structure:
     ...
     ```
   - `child_plan`: the string `"child-plan.md"` if exit state is `next-plan`, otherwise `null`. Use the logical name, not the file path.
