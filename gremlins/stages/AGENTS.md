...
- `plan` — local planning: `resolve-plan-input` → `plan` (agent) → `update-description`. Agent output: `plan-document: file://session/plan.md`. The agent stage has `skip_if_exists: plan-document`, so a resumed run skips the LLM if a non-empty `plan.md` already exists. Any pipeline using this recipe must declare `bootstrap.source` for `plan` and `instructions` — that block is the canonical source of the `--plan`/`--instructions` flags the recipe depends on (`resolve-plan-input` consumes the `plan` artifact; `plan-document` is the agent's output key).
- `plan_gh` — GitHub planning: `resolve-plan-input` → `plan` (agent) → `publish-as-issue` → `update-description`. The agent writes `plan-document: file://session/plan.md` and has `skip_if_exists: plan-document`; `publish-as-issue` has `skip_if_exists: plan-issue-number` (idempotency guard against duplicate issues on resume) and writes `plan-document?: gh://issue/{read:plan-issue-number}`. Same `bootstrap.source` requirement as `plan`.
