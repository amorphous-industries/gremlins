...
impl ApiKeys {
    /// Load from `user_config_root() / "api_keys.json"`.
    pub fn load() -> Result<Self, ApiKeysError> {
        let path = user_config_root().join("api_keys.json");
        let keys = parse_api_keys(&path)?;
        Ok(ApiKeys { keys })
    }
